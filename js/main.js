...
(function(window, $) {
	var ajaxLag = 3000,
		timeout = 20000,
		isGettingData = false;
	var color = ["F6402B", "EB1461", "9C1AB2", "6733BB", "1193F5", "46AF4A", "FFC100", "7A5447", "5F7C8C"];
	...
	function transfer(content) {
		...
		for (var i in list)
			content = content.replace(
				RegExp("\\/\\:" + list[i][0], "g"),
				'<img src="./source/plugin/live_party/img/face/' + list[i][1] + '.gif">'
			);
		// Emoji
		var matches = content.match(/[\ue000-\uefff]/g);
		for (var i in matches) {
			var after = escape(matches[i]).replace("%uE", "").toLowerCase();
			content = content.replace(
				RegExp(matches[i], "g"),
				'<img src="./source/plugin/live_party/img/emoji/e' + after + '.png">'
			);
		}
		return content;
	}
	...
	function getData() {
		var ver = -1;
		var url = location.href, t;
		url = url.split("?")[1];
		if (url) url = url.split("&");
		for (var i in url)
			if ((t = url[i].split("="))[0] == "ver") {
				ver = t[1];
				break;
			}
		$.ajax({
			...
			data: JSON.stringify({
				m: "getPWData",
				ver: ver,
				lastpos: (LastPosition == -1) ? -50 : LastPosition,
				avatar: 0,
				id: 0
			}),
			...
			success: function(d) {
				var isFirstLoad = (LastPosition == -1);
				if (!d || !(d.maxPos) || d.maxPos < 0) {
					if (isFirstLoad) loadComplete();
					return;
				}
				LastPosition = parseInt(d.maxPos);
				if (LastPosition < 0) LastPosition = 0;
				if (d.d) {
					var ts = '';
					$floatDM = $("#floatDM");
					for (var i in d.d) {
						d.d[i].cont = transfer(d.d[i].cont);
						Messages[i] = d.d[i];
						// 统计上墙的用户
						if (!UniqueUsers[d.d[i].user]) {
							UniqueUsers[d.d[i].user] = {
								username: d.d[i].user,
								avatar: d.d[i].avatar,
								color: color[parseInt(Math.random() * color.length)]
							};
						}
						ts += '<div><span style="color:#' + UniqueUsers[d.d[i].user].color + '">' + d.d[i].user + '</span>：' + d.d[i].cont + '</div>';
						if (!isFirstLoad) addDM(i);
					}
	...
	function addDM(id) {
		...
		var mini = 0;
		for (var i = 0; i < 14; ++i) {
			if (verticalPosition[i] < min) {
				min = verticalPosition[i];
				mini = i;
			}
		}
		++verticalPosition[mini];
		var $dm = $('<div line="' + mini + '" class="floatDM" style="top:' + (mini * 28) + 'px;left:640px">' + Messages[id].cont + '</div>');
		$dm.appendTo($floatDM);
		var width = $dm.width();
		setTimeout(function () {
			$dm.animate({
	...
	function loadComplete() {
		$("#live_loading").remove();
		isFirstLoad = false;
		for (var i = 0; i < 14; ++i) verticalPosition.push(0);
	}
	getData();
